...
def quicksort(list, ranks, n):
    if n < 0:
        return []
    elif n < 2:
        return list
    else:
        if ranks[list[n - 1]] < ranks[list[n - 2]]:
            list[n - 1], list[n - 2] = list[n - 2], list[n - 1]
        return quicksort(list, ranks, n - 1)

def ordered_search(index, ranks, keyword):
    if keyword in index:
        smaller, biggest, equal = [], [], []
        pivot = index[keyword][len(index[keyword]) / 2]
        pivot_value = ranks[pivot]
        for url in index[keyword]:
            if ranks[url] < pivot_value:
                smaller.append(url)
            elif ranks[url] == pivot_value:
                equal.append(url)
            else:
                biggest.append(url)
        return quicksort(smaller, ranks, len(smaller)) + equal + quicksort(biggest, ranks, len(biggest)) 
    else:
        return None
